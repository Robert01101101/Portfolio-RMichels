...
Partial::build('projectPageLanding', 
  ["project" => $project, 
  "description" => "Scan Share is a feature of the Clirio product suite, for sharing photogrammetry scans quickly, and displaying them in a webviewer.",
...
<div id="projContent"> <!--___________ Proj Content ____________-->
  ...
  <section class="sectionText">
    ...
    <p>When a user opens the share panel for a scan, the client checks whether there are existing links. If there are, the existing links are provided for the user to share. For any new configuration of a share that hasn't been created yet, the user can press generate, and a request will be sent to a backend API implemented by <a href="https://www.linkedin.com/in/timthibault/" target="_blank">Timothy Thibault</a>. The returned result includes a URL for the client to display to the user, and to include in native share options for each platform.</p>
    ...
    <p>An interesting change to my approach to writing controllers was first tested in this feature, after a discussion I had with my teammate <a href="https://www.linkedin.com/in/toniostillman" target="_blank">Tonio Stillman</a>. In the past, I would write public functions for button clicks, and hook them up in the inspector via the button's <code>OnClick()</code> event. Here, I instead set all the event listeners in the controller itself, kept all the corresponding listener functions private, and simplified the inspector setup so that each button only gets assigned once to the controller, and no other reference is needed. This makes setup in the editor less error-prone and maintainable, as it requires less understanding of the code.</p>
  </section>
  ...
  <section class="sectionMedia">
    <div class="divText">
      <h2>Code Sample</h2>
      <p>The code snippet below contains the code related to loading the 3D Model inside the webviewer, using Three.js. It's a small section of the actual file, but it shows some of the solutions to challenges encountered, such as the modified process for loading the textures defined in the MTL file using SAS tokens, and functions to help configure the viewer based on the model dimensions.</p>
    </div>
