...
<div id="projContent"> <!--___________ Proj Content ____________-->
  ...
  <section class="sectionText">
    ...
    <p><?php echo _("Future Earth was created as an exercise in designing and developing immersive environments."); ?></p>
    <p><?php echo _("With the concept design, we aimed to create a VR adventure game that encourages sustainable behaviors in players. This is achieved by placing the player in a dystopian futuristic environment and giving them the power to radically transform it for the better through gameplay."); ?></p>
  </section>
  ...
  <section class="sectionText">
    ...
    <h3><?php echo _("Story"); ?></h3>
    <p><?php echo _("Set in a dystopian future, you are the first human to walk planet earth in a long time."); ?></p>
    <p><?php echo _("Your mission objective is to plant and sustain 100 trees. Whether or not humanity will attempt to return to earth hinges upon your success."); ?></p>
  ...
  <section class="sectionText">
    ...
    <p><?php echo _("The project was developed in 4 months by a team of 4. I was the project manager, as well as one of the VR / game designers & developers."); ?></p>
    <p><?php echo _("After deciding to create a project for impact with the theme of sustainability, we started to create a game. Our goal was to create a VR game that appeals to players without particular interest in sustainability, to reach a broader audience."); ?></p>
    <p><?php echo _("As the PM, I split the concept design into a list of core and bonus features. The core design featured a small game, where players fight rogue robots, collect their parts, and build water filtering technology that helps trees to grow. This core prototype was implemented within the first three months."); ?></p>
    <p><?php echo _("After a round of usertesting, we proceed to implement updated features in response to our testing results. These included measures to better communicate the narrative, such as a major update to the level design and the implementation of an aesthetic change as the player plants more trees. We also had time to implement some of our bonus features, which were mostly aesthetic improvements such as the addition of VFX, better models, and multiple collectible parts."); ?></p>
  </section>
  ...
  <!----- Content / Text ----->
  <section class="sectionText">
    <h2><?php echo _("The Code"); ?></h2>
    <p><?php echo _("My programming efforts for this project focussed on the more unusual features of our game, such as spare part collection, item building, UI interactions, story sequencing and more. Another interesting feature I implemented was the trees, which required animations for growth and monitoring of their health based on the available water supply. In order to achieve fine control over the growth behavior, I used a Coroutine, which is also used to control dying trees. Instead of using an Unity animation I implemented smooth growth by affecting scale and rotation with the help of "); ?><code>Mathf.SmoothStep()</code><?php echo _(" and "); ?><code>Mathf.Sin()</code><?php echo _(" for smoothing. A challenge that came up was a bug where birds would land at the position of the tree's origin. The fix was simple: disabling their perch-targets until the trees had finished growing, and only then enabling them, so that their target positions were correctly located at the fully grown branches."); ?></p>
  </section>
